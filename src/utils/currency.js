...
const loadPersistentCache = () => {
    try {
        ...
        if (stored) {
            ...
            if (Date.now() - parsed.timestamp < 1000 * 60 * 60 * 24) { // 24h max age for safety
...
const getCrossRate = (rates, baseCode, targetCode) => {
    ...
    if (direct) {
        if (direct.currencyCodeA === baseNum && direct.currencyCodeB === targetNum) {
            ...
            else if (direct.rateBuy && direct.rateSell) rate = (direct.rateBuy + direct.rateSell) / 2;
...
export const fetchExchangeRate = async (base, target, isCrypto = false) => {
    ...
    try {
        ...
        if (isCrypto || ['bitcoin', 'ethereum', 'tether', 'btc', 'eth', 'usdt'].includes(base.toLowerCase())) {
             const coinId = base.toLowerCase().replace('btc','bitcoin').replace('eth','ethereum').replace('usdt','tether');
