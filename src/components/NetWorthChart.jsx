...
export default function NetWorthChart({ data, currency, formatMoney }) {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-6">
            <h3 className="font-bold text-sm text-slate-500 mb-4 uppercase tracking-wider">History</h3>
                    ...
                    {validData.map((d, i) => {
                        ...
                        return (
                            <g key={i} className="group/point">
                                ...
                                {/* Видима точка */}
                                <circle cx={x} cy={y} r="3" className="fill-white stroke-emerald-500 stroke-2 opacity-0 group-hover/group:opacity-100 transition-opacity" />
                                
                                {/* Tooltip */}
                                <g className="opacity-0 group-hover/point:opacity-100 transition-opacity pointer-events-none">
                                    <rect x={x - 20} y={y - 25} width="40" height="18" rx="4" fill="#1e293b" />
                                    <path d={`M${x},${y-7} L${x-4},${y-12} L${x+4},${y-12} Z`} fill="#1e293b" />
                                    <text x={x} y={y - 14} fontSize="5" fill="white" textAnchor="middle" fontWeight="bold">
                                        {formatMoney(d.total, currency).replace(currency, '').trim()}
                                    </text>
                                </g>
                            </g>
                        );
