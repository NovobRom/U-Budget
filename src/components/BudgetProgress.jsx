...
export const BudgetProgress = ({ categories, transactions, limits, currency, formatMoney, onOpenSettings, label }) => {
    ...
    const categoriesWithLimits = categories.filter(c => limits && limits[c.id] > 0 && c.type === 'expense');
    ...
    return (
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-6">
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-slate-900 dark:text-white">{label || 'Expense Limits'}</h3>
                <button onClick={onOpenSettings} className="text-slate-400 hover:text-blue-500"><Settings size={16}/></button>
            </div>
