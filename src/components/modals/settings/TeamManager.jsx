        ...
        <div className="mb-6">
            ...
            <div className="space-y-2">
                {allowedUsers.map((userItem) => {
                    ...
                    const isGenericName =
                        displayName?.startsWith('User ') || displayName === 'Unknown User';
                    const displayLabel = isGenericName && !isCurrentUser ? 'Partner' : displayName;

                    return (
                        <div
                            key={uid}
                            className={`flex items-center justify-between p-3 rounded-xl border ${isCurrentUser ? 'bg-blue-50 border-blue-100 dark:bg-blue-900/20 dark:border-blue-800' : 'bg-slate-50 border-slate-100 dark:bg-slate-800 dark:border-slate-700'}`}
                        >
                            <div className="flex items-center gap-3 overflow-hidden">
                                <div className="relative">
                                    <div
                                        className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold uppercase shrink-0 text-white overflow-hidden ${isOwner ? 'bg-amber-500 shadow-amber-500/30 shadow-md' : isCurrentUser ? 'bg-blue-500' : 'bg-purple-500'}`}
                                    >
                                        {photoURL ? (
                                            <img
                                                src={photoURL}
                                ...
                                <div className="flex flex-col overflow-hidden">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate max-w-[150px]">
                                            {isCurrentUser
                                                ? `${t.you_label} (${displayLabel})`
                                                : displayLabel}
                                        </span>
                                        {isOwner && (
                                            <span className="text-[9px] font-bold bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 px-1.5 py-0.5 rounded uppercase tracking-wide">
                                                {t.owner_label}
                                            </span>
                                        )}
                                    </div>
