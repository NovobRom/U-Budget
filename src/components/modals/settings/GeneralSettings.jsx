...
const FlagUA = () => (
    <svg
        ...
        height="15"
        viewBox="0 0 24 18"
        className="rounded-sm shadow-sm inline-block mr-2"
    >
        <rect width="24" height="9" fill="#0057B8" />
                ...
                <div className="grid grid-cols-3 gap-2">
                    {['en', 'ua', 'pl'].map((l) => (
                        <button
                            ...
                            `}
                        >
                            {l === 'ua' ? <FlagUA /> : l === 'en' ? <FlagGB /> : <FlagPL />}
                            {l.toUpperCase()}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex bg-slate-200 dark:bg-slate-900 p-1 rounded-xl">
                    <button
                        ...
                        className={`
                            ...
                            ${!darkMode ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500'}
                        `}
                    >
                        <Sun size={14} /> {t.light}
                    </button>
                    <button
                        ...
                        className={`
                            flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 
                            ${darkMode ? 'bg-slate-700 shadow-sm text-white' : 'text-slate-500'}
                        `}
                    >
                        <Moon size={14} /> {t.dark}
                    </button>
