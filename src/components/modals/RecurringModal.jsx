...
export default function RecurringModal({ isOpen, onClose, transactions, onAdd, formatMoney, currency, t }) {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl p-6 shadow-xl border border-slate-200 dark:border-slate-700 max-h-[80vh] overflow-y-auto">
                ...
                <div className="space-y-3">
                    {uniqueRecurring.length > 0 ? uniqueRecurring.map(tx => (
                        <div key={tx.id} className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center group">
                            <div>
