                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            ...
                            className={`py-2 rounded-xl text-sm font-bold border-2 ${type === 'expense' ? 'border-red-500 bg-red-50 text-red-600' : 'border-transparent bg-slate-100 text-slate-500'}`}
                        >
                            {t.expense}
                        </button>
                        <button
                            ...
                            className={`py-2 rounded-xl text-sm font-bold border-2 ${type === 'income' ? 'border-green-500 bg-green-50 text-green-600' : 'border-transparent bg-slate-100 text-slate-500'}`}
                        >
                            {t.income}
                        </button>
                    </div>
                        ...
                        <div className="flex gap-2 flex-wrap h-32 overflow-y-auto">
                            {CATEGORY_ICONS_SELECTION.map(({ id, icon: Icon }) => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => setIcon(id)}
                                    className={`p-3 rounded-xl ${icon === id ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-slate-100 text-slate-400'}`}
                                >
                                    <Icon size={18} />
                                </button>
                            ))}
